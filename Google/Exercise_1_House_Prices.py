...
# of a house according to a simple formula.
...
# You don't have to give the answer 400...it might be better to 
# create something that predicts the number 4, and then your answer is in the 
...
xs = np.array([0.0, 1.0, 3.0, 4.0, 6.0, 8.0], dtype = float)
ys = np.array([50.0, 100.0, 200.0, 250.0, 350.0, 450.0], dtype = float)
model.fit(xs, ys, epochs = 500)
print(model.predict([7.0]))

# NOTE: using values that are not similar seemed to throw off our model. The error was growing exponentially every epoch.